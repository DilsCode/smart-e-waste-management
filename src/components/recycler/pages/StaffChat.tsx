...
const StaffChat = () => {
  ...
  const getStaffName = (pickupId: string) => {
    ...
    if (!pickup || !pickup.requester_id) return 'Staff Member';
      ...
      <div className="lg:col-span-1 bg-white dark:bg-gray-800 rounded-[2rem] border border-gray-100 dark:border-gray-700 overflow-hidden flex flex-col">
        ...
        <div className="flex-grow overflow-y-auto p-4 space-y-3">
          {activePickups.length > 0 ? activePickups.map(pickup => (
            ...
            >
              <div className="font-bold truncate">{getStaffName(pickup.id)}</div>
              <div className="text-[10px] opacity-70 uppercase tracking-widest mt-1">Pickup ID: {pickup.id}</div>
      ...
      <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-[2rem] border border-gray-100 dark:border-gray-700 overflow-hidden flex flex-col">
        {selectedPickupId ? (
          <>
            <div className="p-6 border-b dark:border-gray-700 flex items-center justify-between bg-gray-50/50 dark:bg-gray-900/50">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 dark:text-white">{getStaffName(selectedPickupId)}</h3>
                  <p className="text-[10px] text-emerald-500 font-bold uppercase tracking-widest">Staff Member</p>
                </div>
              </div>
            </div>
          ...
          <div className="flex-grow flex flex-col items-center justify-center text-gray-400 p-10 text-center">
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Staff Messages</h3>
            <p className="max-w-xs">Select an active pickup to coordinate with the staff member.</p>
          </div>
        )}
      </div>
    </div>
